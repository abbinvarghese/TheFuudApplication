...
-(void)toolbarDoneClicked{
    ...
    BOOL phoneValidates = [phoneTest evaluateWithObject:tagInputField_.text];
    
    if (tagInputField_.text.length == 0){
        [self endEditing:YES];
    }
    else if (phoneValidates) {
        ...
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"You have entered an invalid phone number" message:@"Please make sure that the Area code is also included" preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *action = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:nil];
        [alert addAction:action];
        
        UITableViewCell *tableCell = (UITableViewCell*)self.superview.superview;
        id view = [tableCell superview];
